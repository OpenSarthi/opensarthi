...
class Session:
    ...
    async def process_incoming(self, data: dict):
        ...
        if msg_type == "client_state":
            ...
            _update_key("google_client_id", "GOOGLE_CLIENT_ID")
            _update_key("google_client_secret", "GOOGLE_CLIENT_SECRET")
            _update_key("twitter_api_key", "TWITTER_API_KEY")
            _update_key("twitter_api_secret", "TWITTER_API_SECRET")
            _update_key("twitter_access_token", "TWITTER_ACCESS_TOKEN")
            _update_key("twitter_access_token_secret", "TWITTER_ACCESS_TOKEN_SECRET")
            _update_key("telegram_bot_token", "TELEGRAM_BOT_TOKEN")
            _update_key("telegram_chat_id", "TELEGRAM_CHAT_ID")
            _update_key("discord_webhook_url", "DISCORD_WEBHOOK_URL")
            _update_key("smtp_host", "SMTP_HOST")
            _update_key("smtp_user", "SMTP_USER")
            _update_key("smtp_password", "SMTP_PASSWORD")
            _update_key("linkedin_access_token", "LINKEDIN_ACCESS_TOKEN")
